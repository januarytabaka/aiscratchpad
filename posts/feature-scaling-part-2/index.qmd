...
In the [previous post](/posts/feature-scaling-part-1/index.qmd) we covered why scaling data is a critical step in machine learning. Now, let's dive into the how. In this guide, we'll explore the three most common scaling techniques and their implementation in both Python and R.
...
First, let's create a simple dataset to help illustrate the concepts.
...
Here’s how you can implement this using the standard libraries in Python and R.
...
cat("\nLearned mean: ", mean_val,
    '\nLearned standard deviation: ', sd_val,
    "\nStandardized Data: [",
...
If you compare the results from Python and R, you'll notice they are slightly different. This isn't an error but a key statistical distinction:
...
To make these concepts clearer, I've plotted the transformation of the same single feature under each technique. We’re not comparing across features; the goal here is to isolate the effect of each scaling method on one variable.
...
Now for the most important part. In this section, we'll demonstrate how the three scaling techniques react when an extreme value is present. This is where you'll really see the practical difference between them and understand the rationale for choosing one over the other.

First, let's create two simple datasets: the original clean data, and a version with a significant outlier.
...
In this post, we've seen three of the most common scaling techniques and explored their behavior on the same feature, including how each one of them responds to outliers.

In **Part 3** of the *Feature Scaling* series, we'll dive into some less common but highly effective transformers: **MaxAbsScaler**, **PowerTransformer**, and **QuantileTransformer** with a brief look at **Normalizer**, which serves a very different purpose but is often mentioned alongside scalers.
